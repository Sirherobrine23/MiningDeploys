...
RUN wget https://github.com/ethereum-mining/ethminer/releases/download/v0.19.0-alpha.0/ethminer-0.19.0-alpha.0-cuda-9-linux-x86_64.tar.gz -O ethminer.tar.gz && \
tar xfz ethminer.tar.gz && rm -rfv ethminer.tar.gz && cd *ethminer* && mkdir -p /Minings/ethminer && cp -rfv * /Minings/ethminer

# Minergate
# minergate-cli --user "$Email" --bcn $(($(nproc) * 2))
RUN wget https://minergate.com/download/xfast-ubuntu-cli/1.2 -O Minergate.deb && dpkg -i Minergate.deb && rm -rfv Minergate.deb

ENV Mining="xmr" Email="srherobrine20@gmail.com" PATH=/Minings/xmrig:/Minings/ethminer:$PATH
